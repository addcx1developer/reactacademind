...
If the <i>"Yes"</i> button is pressed, the `h1` element should receive `color: "green"` as an inline style. If the <i>"No"</i> button is pressed, `color: "red"` should be applied.

Initially, when no button has been clicked yet, the `color` should be set to `"white"`.

<b>Important:</b> You must use these specific colors (`"green"`, `"red"`, `"white"`) - don't use any hex code or slight variations of these colors!
